...
### Créer un administrateur (CLI)

Pour créer un compte administrateur sans le coder en dur, utilisez le mode CLI prévu :
...
|--------|-------------|-------------|
| `--email` | ✅ Oui | Email de connexion de l'admin |
| `--nom` | ❌ Non (défaut `Admin`) | Nom |
| `--prenom` | ❌ Non (défaut `Systeme`) | Prénom |
| `--telephone` | ❌ Non (défaut `000000000`) | Téléphone |
| `--password` | ❌ Non | Mot de passe. S'il n'est pas fourni, un mot de passe sécurisé est généré et affiché dans la console |

> ℹ️ Le script utilise le même service de création d'utilisateur que l'API (`IUserManagementService`), donc toutes les validations habituelles s'appliquent (email unique, etc.).
